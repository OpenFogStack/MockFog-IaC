...
class main:
    ...
    def __init__(self, agentIPpath, testMode):
        ...
        if testMode == 'remote':
            self.localHostName = 'Nodemanager'
        elif testMode == 'local':
            self.localHostName = 'Client'
    ...
    def getIPAdress(self):
        return (([ip for ip in
                ...
                if not ip.startswith('127.')] or [[(s.connect(('8.8.8.8'
    ...
    def pingNode(self, ip):
        count = 4
        proc = subprocess.Popen('ping -c {0} -w 100 {1}'.format(count,
                                ip), shell=True,
        ...
        # self.printLine("row", ["RESULT ENDS HERE"])
            ...
            for vals in valsArr:
                valsTmp = vals.split(' ')
                if valsTmp[1] == 'packets':
                    valsOut.append(valsTmp[0])
                if valsTmp[1] == 'received':
                    valsOut.append(valsTmp[0])

            # if valsTmp[0] == "time": valsOut.append(valsTmp[1])

                if valsTmp[0].endswith('%'):
                    valsOut.append(valsTmp[0])
    ...
    def printLine(
        ...
        if testMode == 'remote':
            formatHTML.printPingTable(style, vals, newLine)
        elif testMode == 'local':
            self.printToShell(style, vals)
    ...
    def pingFromLocal(self):
        sys.stdout.flush()
        self.printLine('headline', ['Pinging created nodes from '
                       + self.localHostName + '...'])
        self.printTableHeader()
        for nodename in self.nodes.keys():
            self.printLine('row', [self.localHostName,
                           self.localHostIP], newLine=False)
            self.printNodeLine(self.nodes, nodename)
            sys.stdout.flush()
            if testMode == 'local':
                self.pingNode(self.nodes[nodename]['public_addr'])
            elif testMode == 'remote':

         # elif testMode == "remote": self.pingNode(self.getLocalIP(self.nodes[nodename]))

                self.pingNode(self.nodes[nodename]['mgmt']['addr'])
        sys.stdout.flush()

    def printNodeLine(self, nodeObj, nodename):
        if testMode == 'local':
            ip = nodeObj[nodename]['public_addr']
        elif testMode == 'remote':

      # elif testMode == "remote": ip = nodeObj[nodename]["mgmt"]["addr"]

            ip = self.getLocalIP(nodeObj[nodename])
            ...
            for dstNode in agents.createHostsList(srcNode, 'list'):
                ...
                try:
                  ...
                  # print res
                    ...
                    self.printLine('row', [packets, received, loss,
                                   delay])
                except Exception, e:
                    self.printLine('error', ['connection Failed.'
                                   + str(e)[0:50]])
...
if __name__ == '__main__':
    ...
    for (opt, arg) in opts:
        if opt == '-h':
            ...
            print 'Defaults are: AgentIPs:' + agentIPpath \
                + ' testMode: ' + testMode
            sys.exit()
        elif opt in '-f':
            agentIPpath = arg
